...
0
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_index.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_list.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_memberlist.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_create.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_invite.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_manage.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/group/group_right.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/common/seccheck.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/common/seditor.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
|| checktplrefresh('./template/default/group/group.htm', './template/default/common/seditor.htm', 1408079191, 'diy', './data/template/5_diy_group_group.tpl.php', './template/mobanbus_boohee', 'group/group')
...
<div id="gh">
<img src="<?php echo $_G['forum']['banner'];?>" alt="" width="980px" height="300px" />
<div class="bm bmw bw0">
<div class="bm_h bw0 cl">
<img src="<?php echo $_G['forum']['icon'];?>" alt="<?php echo $_G['forum']['name'];?>" width="230" height="230" style="margin-top:-180px; margin-left:740px; z-index:10; position:absolute;" />

<?php if($status != 2 && $status != 3) { ?>
<div class="tb cl<?php if($action != 'manage') { ?> mbm<?php } ?>">
<?php if(in_array($_G['adminid'], array(1,2))) { ?><span class="a bw0_all y xi2" style="margin-right:300px;"><a href="javascript:;" onclick="showWindow('grecommend<?php echo $_G['fid'];?>', 'forum.php?mod=group&action=recommend&fid=<?php echo $_G['fid'];?>');return false;">推荐到版块</a></span><?php } ?>
<ul id="groupnav">
...
已加入该<?php echo $_G['setting']['navs']['3']['navname'];?>，等待创始人审核中...
...
<?php if(CURMODULE == 'group') { ?><?php if(!empty($_G['setting']['pluginhooks']['group_top'])) echo $_G['setting']['pluginhooks']['group_top'];?><?php } else { ?><?php if(!empty($_G['setting']['pluginhooks']['forumdisplay_top'])) echo $_G['setting']['pluginhooks']['forumdisplay_top'];?><?php } if($action == 'index' && $status != 2 && $status != 3) { if($status != 2) { if($livethread) { ?>
...
<a href="home.php?mod=space&amp;uid=<?php echo $user['uid'];?>" title="<?php if($user['level'] == 1) { ?>创始人<?php } elseif($user['level'] == 2) { ?>副创始人<?php } if($user['online']) { ?> 在线<?php } ?>" class="avt" c="1">
<?php if($user['level'] == 1) { ?>
<em class="gm"></em>
...
<p class="d">注：<?php echo $_G['setting']['navs']['3']['navname'];?>关闭后只有创始人才可以浏览</p>
<?php } ?>
</td>
</tr>
...
<div class="bm_c">
<ul class="ml mls cl"><?php if(is_array($adminuserlist)) foreach($adminuserlist as $user) { ?><li>
<a href="home.php?mod=space&amp;uid=<?php echo $user['uid'];?>" title="<?php if($user['level'] == 1) { ?>创始人<?php } elseif($user['level'] == 2) { ?>副创始人<?php } if($user['online']) { ?> 在线<?php } ?>" class="avt">
<?php if($user['level'] == 1) { ?>
<em class="gm"></em>
<?php } elseif($user['level'] == 2) { ?>
<em class="gm" style="filter: alpha(opacity=50); opacity: 0.5"></em>
...
<div class="mtm"><h2>温馨提示：</h2><p><?php echo $_G['setting']['navs']['3']['navname'];?>转让成功后，您依然是该圈子的创始人，但不再是<?php echo $_G['setting']['navs']['3']['navname'];?>创始人，所以可自由退出该<?php echo $_G['setting']['navs']['3']['navname'];?>。</p><p>转让后无法直接恢复圈子，需被接收者主动操作才可以再次转回，请谨慎操作。</p></div>
</div>
...
<a href="home.php?mod=space&amp;uid=<?php echo $user['uid'];?>" title="<?php if($user['level'] == 1) { ?>设为创始人<?php } elseif($user['level'] == 2) { ?>设为副创始人<?php } if($user['online']) { ?> 在线<?php } ?>" class="avt"><?php echo avatar($user['uid'], 'small'); ?></a>
<p><a href="home.php?mod=space&amp;uid=<?php echo $user['uid'];?>"><?php echo $user['username'];?></a></p>
...
<div style=" width:240px; background-color:#EAEAEA; float:right;  margin-right:2px;"><?php if($action == 'index') { ?>
<div>
<h1 style="font-size:18px;"><?php echo $_G['forum']['name'];?></h1>
</div>
<div style="padding-top:20px; border:2px solid #D5D5D5; height:auto;">
<?php if($_G['forum']['description']) { ?><div class="pbn"><?php echo $_G['forum']['description'];?></div><?php } ?>
</div>
<div style=" margin-top:10px; padding-top:5px; border:1px solid #D5D5D5; height:20px;">
<a href="home.php?mod=spacecp&amp;ac=favorite&amp;type=group&amp;id=<?php echo $_G['forum']['fid'];?>&amp;handlekey=sharealbumhk_<?php echo $_G['forum']['fid'];?>&amp;formhash=<?php echo FORMHASH;?>" id="a_favorite" onclick="showWindow(this.id, this.href, 'get', 0);" title="收藏" class="fa_fav">收藏</a>
<?php if($status == 'isgroupuser' && helper_access::check_module('group')) { ?>
<span class="pipe">|</span>
<a href="javascript:;" onclick="showWindow('invite','misc.php?mod=invite&action=group&id=<?php echo $_G['fid'];?>')" class="fa_ivt">邀请好友</a><?php } ?>
<span class="pipe">|</span>
<?php if($_G['current_grouplevel']['icon']) { ?><img src="<?php echo $_G['current_grouplevel']['icon'];?>" title="<?php echo $_G['setting']['navs']['3']['navname'];?>等级: <?php echo $_G['current_grouplevel']['leveltitle'];?>" class="vm"> <?php } ?>积分: <?php echo $_G['forum']['commoncredits'];?>
</div>
...
<a href="home.php?mod=space&amp;uid=<?php echo $user['uid'];?>" title="<?php if($user['level'] == 1) { ?>创始人<?php } elseif($user['level'] == 2) { ?>副创始人<?php } elseif($user['level'] == 3) { ?>明星成员<?php } if($user['online']) { ?> 在线<?php } ?>" class="avt" c="1">
<?php if($user['level'] == 1) { ?>
<em class="gm"></em>
<?php } elseif($user['level'] == 2) { ?>
<em class="gm" style="filter: alpha(opacity=50); opacity: 0.5"></em>
