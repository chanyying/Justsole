...
function dump($vars,$level=1,$pre='')  {
	...
	if($level ==1){
		...
		if(gettype($vars) == 'array'){
			foreach($vars as $k=>$v){
				if(gettype($v)== 'array'){
					echo $block."<b style='color:sienna'>©À-['{$k}'] Array Level: {$level}</b>\r\n";
					$_level = $level +1;
					dump($v,$_level,"['{$k}']");
				}else{
					echo $block."<span style='color:blue'>©À-['{$k}']={$v}</span>\r\n";
				}
			}
		}else{
			echo "  ©À-".$vars."\r\n";
	...
	elseif($level >1){
		...
		if(gettype($vars)== 'array'){
			foreach($vars as $_k=>$_v){
				if(gettype($_v)== 'array'){
					echo $_block."<b style='color:sienna'>©À-{$pre}['{$_k}'] Array Level: {$level}</b>\r\n";
					$_level = $level +1;
					dump($_v,$_level,$pre."['{$_k}']");
				}else{
					echo $_block."<span style='color:blue'>©À-{$pre}['{$_k}']={$_v}</span>\r\n";
				}
			}
		}else{
			echo $_block."<span style='color:green'> ©À-{$pre}['{$pre}']={$vars}</span>\r\n";
		}
	}
}
...
function upload_file($file_name , $up_file_dir,$_w='500',$_h='500',$_cut='0'){
	...
	if($_FILES[$file_name]['size']){
		...
		$sqldirname = $sale_config['siteurl'].$attachurl.$upfileDir.date('Ym').'/'.date('d').'/';
