...
if($_GET['coin'] > $config['points_user'] && $_G['uid']) {
...
if($num != 0) {
	...
	*/
	require_once libfile('function/discuzcode');
		...
		if($config['openmdeltw'] == 1 && empty($_G['uid'])) {
			$puid = 0;
		}else{
			$puid = $_G['uid'];
		}
		$ask_data = array(
			...
			'posttime'=>$filetime,
			'ishidden' => addslashes($_GET['ishidden']),
		);	
/* 	}	 */
...
/* 	if($_GET['amod'] == 'edit') {
		...
		//同步贴子，免登录发贴不同步
		if($config['wtttz'] && $wtid && $_G['uid']){
			if($ask_data[coin] > 0) {
				DB::query("INSERT INTO ".DB::table('forum_thread')." (fid, posttableid, readperm, price, typeid, sortid, author, authorid, subject, dateline, lastpost, lastposter, displayorder, digest, special, attachment, moderated, highlight,  isgroup) VALUES ('".intval($config['tbfid'])."', '0', '0', '".$ask_data[coin]."', '', '0', '".$_G['username']."', '".$_G['uid']."', '".$ask_data['subject']."', '".$_G[timestamp]."', '".$_G[timestamp]."', '".$_G['username']."', '0', '0', '3', '0', '1', '1','0')");
			}else{
			...
			$pid = insertpost(array('fid' => intval($config['tbfid']),'tid' => $tid,'first' => '1','author' => $_G['username'],'authorid' => $_G['uid'],'subject' => $ask_data['subject'],'dateline' => $_G['timestamp'],'message' => $ask_data['message'],'useip' => $_G['clientip'],'invisible' => '0','anonymous' => '0','usesig' => '0','htmlon' => '1','bbcodeoff' => '0','smileyoff' => '0','parseurloff' => '0','attachment' => '0',));
	...
	if ($rewrite == '1'){
		showmessage('您的问题已提交成功，很快就会收到解答的哦~', 'wenda/'.$wtid, array(), array('showdialog' => true, 'locationtime' => true));
	}else{
		showmessage('您的问题已提交成功，很快就会收到解答的哦~', 'plugin.php?id=evinm_wenda:list_article&qid='.$wtid, array(), array('showdialog' => true, 'locationtime' => true));
	}
?>